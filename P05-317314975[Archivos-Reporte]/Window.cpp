...
int Window::Initialise()
{
	...
	//CREAR VENTANA
	mainWindow = glfwCreateWindow(width, height, "CMEP - Práctica 5: Optimización y Carga de Modelos", NULL, NULL);
...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (key == GLFW_KEY_G) {
		...
		if (theWindow->articulacion2 > 60.0f) theWindow->articulacion2 = 60.0f; // Tope máximo de apertura
	}
	if (key == GLFW_KEY_B) {
		theWindow->articulacion2 -= 2.0f; // Velocidad al cerrar
		if (theWindow->articulacion2 < 0.0f) theWindow->articulacion2 = 0.0f; // Tope mínimo (cerrado)
	}
