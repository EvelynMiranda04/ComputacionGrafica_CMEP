...
int Window::Initialise()
{
	...
	//CREAR VENTANA
	mainWindow = glfwCreateWindow(width, height, "CMEP - Practica 09: Animacion Basica", NULL, NULL);
...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	// [M]: Reiniciar el recorrido del coche (Ida y Vuelta)
	if (key == GLFW_KEY_M && action == GLFW_PRESS) {
		theWindow->reiniciaCoche = true;
	}

	// [N]: Gatillo para TiroD8 (Dejado preparado)
	if (key == GLFW_KEY_N && action == GLFW_PRESS) {
		theWindow->tiroD8 = !theWindow->tiroD8; // Funciona como interruptor (On/Off)
	}
